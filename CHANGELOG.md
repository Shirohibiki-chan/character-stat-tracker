...
### Additions
- **Solo and Group columns in bot table:** when any of your bots has group chat breakdown data captured, two columns appear next to Messages — Solo and Group — showing the split counts and gains since the previous snapshot. Both are sortable. The columns are hidden entirely when no breakdown data exists.
