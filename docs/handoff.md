...
**Third-round fix (2026-05-23) — row banding CSS variable:**
`.row-banded` was using `rgba(255, 255, 255, var(--table-banding-alpha))`. LightningCSS preserves the CSS variable reference unresolved; if the browser fails to compute it, `rgba()` produces an invalid value and the background is transparent — banding disappears and column tints appear darker (banding normally shows through the semi-transparent tinted cells, adding subtle brightening). Fixed by hardcoding `rgba(255, 255, 255, 0.030)` directly, matching the pattern of every other alpha-sensitive rule in the stylesheet.

**Grid banner pfp squish fix (2026-05-23):**
The bot grid banner was an `<img w-full h-full object-cover>` inside a fixed 120px container. During progressive image loading the browser briefly renders partial data before `object-cover` crops correctly — causing a visible squish on refresh. Fixed by switching to CSS `background-image` on the container div; background images don't participate in layout so no layout shift occurs during loading.
