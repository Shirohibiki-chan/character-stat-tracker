...
export default function CompareGains({ myBotList, friendBotList, myName, friendName, myColor, friendColor }) {
  const [metric, setMetric] = useState('messages')
  const [days, setDays] = useState(30)
  const [topN, setTopN] = useState(15)

  const data = useMemo(() => {
    const tag = (list, owner) =>
      list
        .map(b => {
          const gain = computeGain(b, days)
          return { ...b, _owner: owner, _val: gain[metric] ?? 0 }
        })
        .filter(b => b._val > 0)

    const all = [...tag(myBotList, 'my'), ...tag(friendBotList, 'friend')]
      .sort((a, b) => b._val - a._val)
    return topN === 0 ? all : all.slice(0, topN)
  }, [myBotList, friendBotList, metric, days, topN])
        ...
        <div className="flex gap-1 p-0.5 bg-surface-alt rounded">
          {WINDOWS.map(w => (
            <button
              ...
              className={`px-2.5 py-1 text-xs font-semibold rounded transition ${days === w.days ? 'bg-surface text-text-primary' : 'text-text-muted hover:text-text-secondary'}`}
            >
              {w.label}
            </button>
          ))}
      ...
      {data.length === 0 ? (
        <div className="flex items-center justify-center py-16">
          <p className="text-text-muted text-sm text-center max-w-xs">
            No gains to show for this window.<br />
            <span className="text-[11px]">Try a wider time range, or add more snapshots.</span>
          </p>
        </div>
      ) : (
        <div className="p-5">
                        ...
                        {METRICS.map(mx => {
                          ...
                          return (
                            <div key={mx.key} className="flex justify-between gap-6 text-sm">
                              ...
                              <span className="num font-semibold" style={{ color: mx.color }}>+{fmtFull(g)}</span>
                            </div>
                          )
                        })}
                      </BotTooltip>
          ...
          <p className="text-[11px] text-text-muted text-center mt-3">
            {m?.label} gained — all bots from both exports · colour = owner
          </p>
        </div>
      )}
    </div>
